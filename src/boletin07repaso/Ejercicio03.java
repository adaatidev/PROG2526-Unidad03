...
public class Ejercicio03 {
	...
	public static void main(String[] args) {
		...
		String textoLargo = "DON QUIJOTE DE LA MANCHA\r\n" + "Miguel de Cervantes Saavedra\r\n" + "\r\n"
				+ "PRIMERA PARTE\r\n"
		...
		String[] palabrasClave = { "QUIJOTE", "MANCHA", "Miguel", "Cervantes", "Sergio", "básico", "hombre", "mundo",
				"claro", "turbio" };

		System.out.print("Contiene palabras: ");

		for (int i = 0; i < palabrasClave.length; i++) {

			if (contienePalabra(textoLargo, palabrasClave[i])) {
		...
		System.out.println("Palabras que aparecen en el texto: "
				+ Arrays.toString(palabrasEncontradas(textoLargo, palabrasClave)));

		System.out.print("Palabras encontradas: ");
		mostrarResultado(palabrasClave);
	...
	public static String[] palabrasEncontradas(String texto, String[] claves) {

		String[] contieneClaves = new String[claves.length];

		for (int i = 0; i < claves.length; i++) {
			if (texto.contains(claves[i])) {
				contieneClaves[i] = claves[i];
			}
		}

		return contieneClaves;

	}

	public static void mostrarResultado(String[] encontradas) {

		String encontradasComas = "";

		if (encontradas.length == 0) {
			System.out.println("No se ha encontrado ninguna.");
		} else {
			for (int i = 0; i < encontradas.length; i++) {
				encontradasComas = encontradas[i].join(", ", encontradas);
			}
		}

		System.out.println(encontradasComas);
