...
public class MensajesGrabados {
	...
	private static final String INVALID_INPUT_MESSAGE = "(Entrada inválida tras normalizar: texto vacío, mensaje vacío o mensaje más largo que el texto.)";
	...
	public static void main(String[] args) {
		...
		analyzeAndPrint(originalText, originalMessage);

		scanner.close();
	}

	/**
	 * Normaliza los datos, valida la entrada, busca las apariciones y muestra los
	...
	private static void analyzeAndPrint(String rawText, String rawMessage) {
		...
		double usagePercentage = calculateUsagePercentage(textCharacters.length, messageCharacters.length,
				occurrenceCount);

		printAnalysisResult(rawMessage, startPositions, usagePercentage, false);
	} // Esta función, para que estuviera de matrícula de honor sería conveniente
		// dividirla en 3 funciones más pequeñas, intentando seguir siempre Clean Code

	/**
	 * Lee una línea completa de texto desde teclado mostrando previamente un
	 * mensaje.
	 *
	 * @param scanner objeto Scanner ya inicializado para leer desde entrada
	 *                estándar
	 ...
	 *
	 * La entrada será válida si: el texto no está vacío, el mensaje no está vacío,
	...
	private static boolean isValidInput(String normalizedText, String normalizedMessage) {
		return normalizedText.length() > 0 && normalizedMessage.length() > 0
				&& normalizedMessage.length() <= normalizedText.length();
	}

	/**
	 * Normaliza un texto convirtiéndolo a minúsculas y eliminando todos los
	 * espacios.
	 *
	 ...
	 * @return {@code true} si ambos caracteres coinciden; {@code false} en caso
	 *         contrario
	 */
	...
	/**
	 * Comprueba si el mensaje completo puede construirse como subsecuencia del
	 * texto a partir de una posición concreta.
	 *
	...
	private static boolean canBuildMessageFrom(char[] textCharacters, char[] messageCharacters, int startIndex) {
		...
		for (int textIndex = startIndex; textIndex < textCharacters.length
				&& messageIndex < messageCharacters.length; textIndex++) {
	 ...
	 *
	 * Este porcentaje se obtiene multiplicando: apariciones * longitud del mensaje
	 * y dividiéndolo entre la longitud total del texto.
	 *
	 * No tiene en cuenta posibles solapamientos reales entre apariciones, que es en
	 * lo que quedamos ayer en clase, creo Aún así no me convence mucho la
	 * implementación de esta funciṕon porque sigo creyendo que podría salir más del
	 * 100% de uso del texto y no tiene sentido.
	 *
	 ...
	 * @param startPositions  posiciones iniciales en las que puede formarse el
	 *                        mensaje
	 * @param usagePercentage porcentaje de uso calculado
	 * @param invalidInput    indica si la entrada era inválida tras la
	 *                        normalización
	 */
	private static void printAnalysisResult(String originalMessage, int[] startPositions, double usagePercentage,
